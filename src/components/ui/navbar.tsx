      ...
      {({ open }: any) => (
        <>
          <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div className="relative mx-2 flex h-16 items-center justify-between">
                    ...
                    {navigation.map((item) => (
                      <a
                        ...
                        className={classNames(
                          item.current ? `bg-gray-900 text-white` : `text-gray-300 hover:bg-gray-700 hover:text-white ${item.name == 'Result' && (session.status=='loading' || session.status == 'unauthenticated') ? "hidden" : ""}`,
              ...
              {session.status==='authenticated' ? <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                {/* Profile dropdown */}
